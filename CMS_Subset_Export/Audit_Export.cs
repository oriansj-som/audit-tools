...
namespace CMS_Subset_Export
{
    ...
    class Audit_Export
    {
        ...
        public int Initialize(string Database, string Agency)
        {
            Initialize(Database);
...
#pragma warning restore CS8602 // Dereference of a possibly null reference.
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine("unable to find AgencyID for provided NickID");
                Environment.Exit(1);
            }
            // Should never be hit
            return 0;
        ...
        public void Generate(FileInfo O, int AgencyID, int Year, string set, string audit_name)
        {

            output = O;
            if (Match.CaseInsensitive("CMS ARC-AMPE", audit_name)) Generate_ARC_AMPE(AgencyID, Year, set);
            else if (Match.CaseInsensitive("CMS MARS-E 2.2", audit_name)) Generate_CMS(AgencyID, Year, set);
            else if (Match.CaseInsensitive("CMS MARS-E 2.0", audit_name)) Generate_CMS(AgencyID, Year, set);
            else
            {
        ...
        private void Generate_CMS(int AgencyID, int Year, string set)
        {
            int worksheet = 2;
            int row = 21;
...
#pragma warning restore CS8602 // Dereference of a possibly null reference.
                ...
                CMS_Control_Page_Generator.CMS_Control_Page_Generator.Add_Control(Report.Workbook.Worksheets[name], i, true);
                worksheet += 1;
...
WHERE AGENCY_ID = '{0}';"
        ...
        private void Generate_ARC_AMPE(int AgencyID, int Year, string set)
        {
            int worksheet = 2;
            int row = 21;
            string sql = string.Format(@"
